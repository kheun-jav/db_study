/*
   1. emp 테이블에서 empno는 사원번호로, ename 사원명, 
     job는 직급으로 별칭을 설정하여 조회하기
*/
SELECT empno 사원번호, ename 사원명, job 직급 FROM emp
/*
	2. dept 테이블에서 deptno 부서, dname 부서명,
	   loc 부서위치로 별칭을 설정하여 조회하기
*/
...
*/
SELECT NAME 이름, weight '몸무게(kg)', major1 학과코드
FROM student
...
*/
SELECT ename 이름, salary 현재급여,
salary*1.05 예상인상급여, deptno 부서코드
FROM emp
WHERE salary >=700
/*
...
weight '몸무게(kg)', height '키(cm)', major1 학과코드
FROM student
...
/*
   ...
   단 생일이 빠른 순서대로 정렬
*/
SELECT NAME 이름, jumin 주민번호, birthday 기준생일,
height '키(cm)', weight '몸무게(kg)'
FROM student
WHERE grade = 1
ORDER BY birthday;
...
*/
SELECT studno 학번, NAME 이름, major1 전공1코드
FROM student
WHERE NAME LIKE '%훈'
ORDER BY grade;
/*
