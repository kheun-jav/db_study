...
SELECT 6003, '이몽룡', 'monglee', POSITION, salary, bonus, deptno, CURRENT_DATE()
FROM professor
WHERE NAME = '박인숙'
SELECT * FROM professor
WHERE NAME IN ('이몽룡', '박인숙')
-- 10. major 테이블에서 code값이 200 이상인 데이터만 major2에 데이터 추가하기
CREATE TABLE major2 AS SELECT * FROM major WHERE CODE >= 200
SELECT * FROM major2
DELETE FROM major2
-- 11.  major2 테이블에 공과대학에 속한 학과 정보만 추가하기 major 테이블 사용
SELECT * FROM major
INSERT INTO major2 SELECT * FROM major WHERE part IN
...
SELECT * FROM emp
UPDATE emp SET salary = (SELECT AVG(salary)*0.8 FROM emp 
								 WHERE deptno = (SELECT deptno FROM emp WHERE eNAME = '박진택')),
